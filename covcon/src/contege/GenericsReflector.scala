...
object GenericsReflector
{
	...
	{
		//解析var对应到class
		//typeVar2Class是Map[TypeVariable[_], Type]类型
		val typeVar2Class = resolveTypeVars(receiverCls)
		val result = new ArrayList[String]()
		//得到c的一个克隆
		val rawParamTypes = c.getParameterTypes
		var pos = 0
		//getGenericParameterTypes其返回的是参数的类型,
		c.getGenericParameterTypes.foreach(pt =>
		{
			val cls =
				if (pt.isInstanceOf[TypeVariable[_]])
				{
					if (typeVar2Class.contains(pt.asInstanceOf[TypeVariable[_]]))
						//将cls赋值为
						typeVar2Class(pt.asInstanceOf[TypeVariable[_]])
					else
						//或者赋值为第一个构造函数
						rawParamTypes(pos)
				}
				...
				{
					var c = getClass(pt)
					// may happen for arrays of unbounded type parameters
					//可能会出现无界类型参数的数组
					if (c == null) c = rawParamTypes(pos)
					c
				}
			result.add(cls.getName)
			pos += 1
	...
	private def resolveTypeVars(cls: Class[_]) =
	{
		...
		//步骤1:遍历类层次结构，将类型vars映射到它们的值(也可以是类型vars)
